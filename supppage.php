<?php
	...
	if($action=="add_supplier"){
		...
		$getsupcurr     = isset($_POST['supcurr']) ? $_POST['supcurr'] : '';
		...
		try {
			...
			try {
			$sql = "INSERT INTO supplier (suppid, suppname, suppcate, supppic, suppadd,supptel,suppemail,suppstts,supprmk,suppcrby,suppcurr) 
					VALUES (:supid, :supname, :supcate, :suppic, :supadd, :suptel, :supemail, :supstts , :suprmk, :supcrby, :supcurr)";
		...
		try {
			$sql = "UPDATE supplier 
					SET suppname = :supname, suppcate = :supcate, supppic = :suppic, suppadd = :supadd, supptel = :suptel, suppemail = :supemail, supprmk = :suprmk, , suppcurr = :supcurr
	...
	$suppliers = $pdo->query("SELECT suppid, suppname, suppcate, scatename, supppic, suppadd, supptel,suppemail, suppstts, supprmk, suppcurr
			...
			<div id="supp-data-container" style="display: none;">
				<?php foreach ($suppliers as $supp01): ?>
					<div class="dept-item" 
						 ...
						 data-curr="<?php echo htmlspecialchars($supp01['suppcurr']); ?>"
						...
						<form method="POST" action="?page=supplier">
					        <div class="row g-3">
								...
								<?php
									...
									<?php
									try {
										$scateStmt = $pdo->query("SELECT curcode, curname, cursymbol FROM pcurrency ORDER BY curcode ASC");
										$suppcurrency = $scateStmt->fetchAll(); 
									} catch (PDOException $e) {
										$suppcurrency = []; 
									}
									// Ensure $current_scate is defined so it doesn't throw an "Undefined variable" notice
									$current_curr = isset($current_curr) ? $current_curr : ''; 
									?>

									<div class="col-md-6">
										<label class="form-label fw-bold" for="form-supcate">Currency  </label>
										<select class="form-select" id="form-supcurr" name="supcurr" required>
											<option value="">-- Select Currency --</option>
											<?php foreach ($suppcurrency as $scurr): ?>
												<option value="<?php echo htmlspecialchars($scurr['curcode']); ?>" 
													<?php echo ($current_curr == $scurr['curcode']) ? 'selected' : ''; ?>>
													<?php echo htmlspecialchars($scurr['curcode'] . ' - ' . $scurr['curname']); ?>
												</option>
											<?php endforeach; ?>
										</select>
									</div>								
...
<script>
	...
	function selectSupplier(suppId) {
		...
		if (targetItem) {
			...
			document.getElementById('form-supcurr').value = data.curr;
	...
	function clearSupplierForm() {
		...
		document.getElementById('form-supcurr').value = '';
